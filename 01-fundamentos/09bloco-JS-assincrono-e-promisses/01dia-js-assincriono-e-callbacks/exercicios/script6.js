...
const sendMarsTemperature = (callback, errorCallback) => {
  setTimeout(() => {
    const sendMessageSuccesPercentage = 60;
    const errorStatus = Math.floor(Math.random() * 101);
    const errorReason = 'Robot não quer';
    if (errorStatus <= sendMessageSuccesPercentage) callback(getMarsTemperature())
    else errorCallback(errorReason)
  }, messageDelay());
};
